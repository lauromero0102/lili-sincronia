...
async function importWorkbook(){
 ...
 try{const workbook=XLSX.read(await file.arrayBuffer(),{type:'array'}),sheet=workbook.Sheets[workbook.SheetNames[0]],rows=XLSX.utils.sheet_to_json(sheet,{header:1,raw:false,defval:'',blankrows:false}),header=findHeaders(rows);if(header.index<0||header.score<2){status.textContent='No se identificaron los encabezados de la plantilla. Incluya al menos nombre del paciente, especialidad, especialista y tiempo quirurgico.';return}const headers=rows[header.index],dataRows=rows.slice(header.index+1).filter(row=>row.some(value=>cleanText(value)));let accepted=0,rejected=0,problems=[];const start=selectedDate();cases=cases.filter(item=>item.source!=='excel');
  dataRows.forEach((row,index)=>{const name=cleanText(valueByAliases(row,headers,importFields.name)),doc=cleanText(valueByAliases(row,headers,importFields.doc)),episode=cleanText(valueByAliases(row,headers,importFields.episode)),cups=cleanText(valueByAliases(row,headers,importFields.cups)),procedure=cleanText(valueByAliases(row,headers,importFields.procedure)),specialist=cleanText(valueByAliases(row,headers,importFields.specialist)),specialty=cleanText(valueByAliases(row,headers,importFields.specialty)),anesthesia=cleanText(valueByAliases(row,headers,importFields.anesthesia)),site=normalizedSite(valueByAliases(row,headers,importFields.site)),statusValue=normalizedPriority(valueByAliases(row,headers,importFields.priority)),minutes=Number(String(valueByAliases(row,headers,importFields.minutes)).replace(',','.')),hasClinicalIdentity=name||doc||episode||cups||procedure||specialist||specialty;if(!hasClinicalIdentity)return;const item={name,specialty,specialist,minutes},missing=missingImportFields(item);if(missing.length){rejected++;if(problems.length<3)problems.push(`fila ${header.index+index+2}: falta ${missing.join(', ')}`);return}cases.push({order:seq++,date:start,site,name,doc,episode:episode||'Sin episodio',status:statusValue,anesthesia,specialty,specialist,minutes,cups:cups||'No informado',procedure,source:'excel'});accepted++});bulkAudit.imported=accepted;bulkAudit.rejected=rejected;bulkAudit.file=file.name;saveImportedCases();plan();status.innerHTML=`<strong>${accepted.toLocaleString('es-CO')} solicitudes importadas y visibles en el cronograma.</strong>${rejected?` ${rejected.toLocaleString('es-CO')} filas fueron omitidas por falta de datos de programacion.${problems.length?` Revise ${problems.join('; ')}.`:''}`:''} Inicio de busqueda: ${start}.`}
